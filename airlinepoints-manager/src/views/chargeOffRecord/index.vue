<template>
  <div class="app-container">
    ...
    <div class="addParking">
      <el-button type="primary" size="small" plain @click="handleAdd" icon="el-icon-circle-plus-outline">新增兑换记录</el-button>
    </div>
    ...
    <el-dialog class="addParkingDialog" :title="titles" :append-to-body="true" :visible.sync="dialogVisible" width="50%" :before-close="handleClose">
      ...
      <el-form :model="parkingForm" ref="parkingForm" label-width="200px" class="demo-ruleForm" size="small">
        ...
        <el-form-item label="优惠卡描述：" prop="cardDesc" :rules="{ required: true, message: '说明不能为空', trigger: 'blur' }">
          <el-input v-model="parkingForm.cardDesc" type="textarea" :rows="2" placeholder="请添加优惠卡描述"></el-input>
        </el-form-item>
    ...
    <el-table
      ...
      <el-table-column prop="title" label="兑换时间" width="150" align="center"></el-table-column>
      <el-table-column prop="parkName" label="兑换商家" width="200" align="center"></el-table-column>
...
<script>
import { getList, addMonthCard, editMonthCard, deleteMonthCard, getParking, getItemMonthCard, editState, getCarType } from '@/api/chargeOffRecord'
...
export default {
  ...
  methods: {
    ...
    //误操作关闭 dialog 的提示
    handleClose(done) {
      this.$confirm('关闭后不会保存编辑的内容，确认关闭？')
        .then(_ => {
          done();
          this.$refs.parkingForm.resetFields();
        })
