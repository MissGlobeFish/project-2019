...
export default class EstateRankPage extends Component {
                ...
                <ScrollView
                    ...
                    <View
                        ...
                        <View
                            ...
                            <Text style={styles.titleStyle}>{realTime ? "地产实时排行榜" : "地产上月排行榜"}</Text>
                            {/* Date */}
                        ...
                        {/* <Button
                            title="Tap me"
                            onPress={() => {
                                // setParams({ realTime: !this.state.realTime})
    ...
    configeDate(isThisMonth, finashCallBcak = ()=>{}) {
      console.log(isThisMonth,'是否本月')
        ...
        var lastMonthStartDate = formatDate(new Date(lastYear, lastMonth, 1));// 上月的年+月+1日
        var lastMonthEndDate = formatDate(new Date(lastYear, lastMonth, getMonthDays(lastMonth)));// 上月的年+月+最后日
        this.setState({
    ...
    loadDatas() {
        ...
        ajax.get('ranking-details', { startDate: startDate, endDate: endDate, type: ranktype.toString() }).then(res => {
