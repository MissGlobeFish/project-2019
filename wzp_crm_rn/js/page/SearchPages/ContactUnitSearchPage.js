...
export default class ContactUnitSearchPage extends Component<Props> {

    static navigationOptions = ({ navigation }) => {
        let unitType = navigation.getParam('unitType')
        return {
            title: '查找' + unitType.name,
                ...
                {searchText.length > 0 && searchResult.length == 0 ? this._emptyComponent() :
                    (<FlatList
                        ...
                        renderItem={({ item }) => (
                            ...
                            >
                                <Text style={styles.titleLab}>{item.name}</Text>
    ...
    search() {
        ...
        let unitType = this.props.navigation.getParam('unitType')
        let self = this
        console.log("搜索", searchText)
        ajax.get('unitSearch', { type: unitType.id, searchKey: searchText, userId: PersistanceManger.ShareInstance().userId })
            .then((res) => {
                self.setState({
                    searchResult: res
                })
    ...
    didSelectRow(dataInfo){
        DeviceEventEmitter.emit(global.NotificationIdentify.unitSearchDidSelect, dataInfo);
