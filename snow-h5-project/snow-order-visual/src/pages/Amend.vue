<template>
	<div class="amend">
		<Products v-if="showProduct" @goBack='goBack'></Products>
		<Amap v-show="showAmap" @goSetup='goSetup'></Amap>
		<div v-show="showAmend" style="padding: 50px 0 36px 0;">
			<div class="top">
				<i class="goBack el-icon-arrow-left" onclick="window.history.go(-1)"></i>
				提交订单
				<el-button id="set_up" type="primary" @click="handleSave" ref="set_up" v-loading="loading" :disabled="disabled">保存</el-button>
			</div>
			<div class="first">
				<h3>订单信息</h3>
				<p>
					<i>订单号：</i>
					<span>{{detail.billNo}}</span>
				</p>
				<p>
					<i>初始账号：</i>
					<span>{{detail.loginName}}</span>
				</p>
				<p>
					<i>初始密码：</i>
					<span>{{detail.password}}</span>
				</p>
				<p>
					<i>所属销售：</i>
					<span>{{detail.salerMan}}</span>
				</p>
			</div>
			<div class="box">
				<p>基本信息</p>
				<input ref="proName" placeholder="新项目 / 门店名称" v-model="detail.proName == ' ' ? '' : detail.proName" :class="[proActive ? errorClass : '']">
				<input ref="cusName" placeholder="公司名称" v-model="detail.cusName == ' ' ? '' : detail.cusName" :class="[cusActive ? errorClass : '']">
			</div>
			<div class="box">
				<div class="title">
					<span>地址信息</span>
					<i class="redact" @click="handleAmap">编辑</i>
				</div>
				<input ref="provinceName" placeholder="省" v-model="provinceName" :class="[provinceActive ? errorClass : '']">
				<input ref="cityName" placeholder="市" v-model="cityName" :class="[cityActive ? errorClass : '']">
				<input ref="areaName" placeholder="区" v-model="areaName" :class="[areaActive ? errorClass : '']">
				<input ref="address" placeholder="详细地址" v-model="address" :class="[addressActive ? errorClass : '']"><br/>
				<input ref="longitude" placeholder="经度" v-model="longitude">
				<input ref="latitude" placeholder="纬度" v-model="latitude">
				<input ref="serTel" placeholder="电话号码" v-model="detail.serTel == ' ' ? '' : detail.serTel" @input="serTel" :class="[serTelActive ? errorClass : '']">
			</div>
			<div class="box">
				<p>联系人信息</p>
				<input ref="linker" placeholder="联系人" v-model="detail.linker == ' ' ? '' : detail.linker" :class="[linkerActive ? errorClass : '']">
				<input ref="position" placeholder="职务" v-model="detail.position == ' ' ? '' : detail.position" :class="[positionActive ? errorClass : '']">
				<input ref="telephone" placeholder="手机" v-model="detail.telephone == ' ' ? '' : detail.telephone" @input="telephone" :class="[telephoneActive ? errorClass : '']">
			</div>
			<div class="box">
				<p>银行账户</p>
				<input ref="openBank" placeholder="开户行名称   如: 中国建设银行成都市高新支行" v-model="detail.openBank == ' ' ? '' : detail.openBank">
				<input ref="bankAcct" placeholder="银行账户" v-model="detail.bankAcct == ' ' ? '' : detail.bankAcct" @input="bankAcct" :class="[bankActive ? errorClass : '']">
				<input ref="acctName" placeholder="账号名称   如：**公司**人" v-model="detail.acctName == ' ' ? '' : detail.acctName">
			</div>
			<div class="product">
				<div class="title">
					<span>选购产品</span>
					<i class="choice" @click="handleProducts">选择</i>
				</div>
				<div class="productBox">
					<ul class="serve" v-show="showServe">
						<b>服务</b>
						<li v-for="item in serveInfo" :key="item.productId">
							<p>
								<i>{{item.productName}}</i>
								<span>{{item.totalAmount}}元 {{item.serverMonth}}个月</span>
							</p>
							<p v-show="item.nextServerAmount">
								<i>续费价格</i>
								<span>{{item.nextServerAmount}}元 12个月</span>
							</p>
						</li>
						<strong>服务总价：{{serveTotalAmount}}元</strong>
					</ul>
					<ul class="facility" v-show="showFacility">
						<b>设备</b>
						<li v-for="item in facilityInfo" :key="item.productId">
							<p>
								<i>{{item.productName}}</i>
								<span>{{item.quantity}}台</span>
							</p>
						</li>
						<strong>押金总价：{{facilityTotalAmount}}元</strong>
					</ul>
				</div>
				<br>
				<div class="foot">
					<strong>总计：{{allTotalAmount}}元</strong>
				</div>	
			</div>
			<div class="card">
				<h3>桌牌设置</h3>
				<div class="checkeBox">
					<span>是否已对接点餐系统</span>
					<el-checkbox v-model="checked" disabled>是</el-checkbox>
					<el-checkbox v-model="checked1" disabled>否</el-checkbox>
				</div>
				<div class="drop">
					<p style="text-align:left;margin-left:10px;">商家系统：</p>
					<div class="dropBox">
						<p ref="caterSystemName" @click="handleSystem" :class="[caterActive ? borderClass : '']">
							{{caterSystemName}}
						</p>
						<ul v-show="showSystem">
							<li v-for="(system, index) in systems" :key="system.label" @click="handleEvent($event,index)">{{system.label}}</li>
						</ul>
					</div>
					<input ref="caterMerchantId" placeholder="餐饮系统ID" v-model="caterMerchantId" :class="[IdActive ? errorClass : '']">
				</div>
			</div>
			<div class="box">
				<p>其他信息</p>
				<span class="span">是否开通支付服务</span>
				<el-radio-group v-model="radio1" @change="handlePay">
					<el-radio :label="1">是</el-radio>
					<el-radio :label="2">否</el-radio>
				</el-radio-group><br/>
				<span class="span">是否需要安装服务</span>
				<el-radio-group v-model="radio2" @change="handleRadio">
					<el-radio :label="3">是</el-radio>
					<el-radio :label="4">否</el-radio>
				</el-radio-group>
				<div class="install" v-show="handleInstall">
					<template>
						<div class="block">
							<el-date-picker :editable="false" v-model="value1" type="date" placeholder="预装日期" :picker-options="pickerOptions"></el-date-picker>
						</div>
					</template>
					<input ref="installLinker" placeholder="安装对接人" v-model="installLinker" :class="[installLinkerActive ? errorClass : '']">
					<input ref="linkerTel" placeholder="对接人电话" v-model="linkerTel" @input="linkerTelephone" :class="[linkerTelActive ? errorClass : '']">
					<textarea ref="environmentDesc" placeholder="其他环境描述" v-model="textarea1"></textarea>
				</div>
			</div>
			<div class="box">
				<p>备注</p>
				<textarea ref="description" placeholder="请输入内容" v-model="textarea2"></textarea>
			</div>
			<div class="box">
				<p>附件上传</p>
				<ul class="imgBox">
					<li v-for="item in file" :key="item.url"><img :src="item.url"></li>
				</ul>
				<el-upload
					list-type="picture-card"
					action="/erp-api/erp/file/upload"
					:data="upLoadData"
					:before-upload="beforeUpload"
					:on-success="UploadSuccess"
					:on-error="UploadError"
					:on-remove="handleRemove">
					<i class="el-icon-plus"></i>
				</el-upload>
				<el-dialog v-model="dialogVisible" size="tiny">
					<img width="100%" :src="dialogImageUrl" alt="">
				</el-dialog>
			</div>
			<el-button class="submit" type="primary" @click="handlePrimary" v-loading="primaryLoading" :disabled="disabled">提 交</el-button>
		</div>
	</div>
</template>

<script>
import Products from '../components/Products'
import Amap from '../components/Amap'

export default {
	name: 'amend',
	data () {
		return {
			userId : window.localStorage.getItem("userId"),
			token : window.localStorage.getItem("token"),
			/* userId : '101508',
            token : '5a5c6c1a-435b-44b4-93a1-1de40c6af8d3', */
			fId: this.$route.query.fId,
            cusType : this.$route.query.cusType,
            upLoadData: {
		        token: window.localStorage.getItem("token"),
		        fileName: ''
		    },
		    checked: true,
		    checked1: false,
			msg: '',
			pay: '',
			install: '',
			radio1: 1,
			radio2: 3,
			textarea1: '',
			textarea2: '',
			provinceName: '',
			cityName: '',
			areaName: '',
			address: '',
			longitude: '',
			latitude: '',
			detail: [],
			productEntity: [],
			serveInfo: [],
			facilityInfo: [],
        	caterSystem: '',
        	caterSystemName: '',
        	caterMerchantId: '',
        	installLinker: '',
			linkerTel: '',
			systems: [],
			period: '',
			serveTotalAmount: null,
			facilityTotalAmount: null,
			allTotalAmount: null,	
			files: [],
			file: [],
			showServe: false,
			showFacility: false,
			handleInstall: false,
			dialogImageUrl: '',
        	dialogVisible: false,
        	showAmap: false,
			showProduct: false,
			showAmend: true,
        	loading: false,
        	primaryLoading: false,
        	showSystem: false,
		    proActive: false,
		    cusActive: false,
			provinceActive: false,
			cityActive: false,
			areaActive: false,
			addressActive: false,
			serTelActive: false,
			linkerActive: false,
			positionActive: false,
			telephoneActive: false,
			bankActive: false,
			installLinkerActive: false,
			linkerTelActive: false,
			IdActive: false,
			caterActive: false,
			disabled: false,
			errorClass: 'error',
			borderClass: 'border',
			value1: '',
			pickerOptions: {//预装日期变量
		        disabledDate(time) {
		            return time.getTime() < Date.now() - 8.64e7;
		        }
	        }
		}
	},
	components: {
        'Products' : Products,
        'Amap' : Amap
    },
	created() {//在实例创建完成后被立即调用
		let that = this;
		let url = '/erp-api/erp/sale/order/detail/new?userId='+ this.userId +'&fId='+ this.fId +'&cusType='+ this.cusType +'&token='+ this.token +'';
		that.$http.get(url).then(function (data) {//请求成功调用的函数
			let info = data.body.data;
			if (info) { 
				let detail = data.body.data;
				that.detail = detail;
				that.productEntity = detail.productEntity;
				if (detail.productEntity) {
					let p = detail.productEntity;
					that.serveTotalAmount = 0;
					that.facilityTotalAmount = 0;
					for (const i in p) {
						let code = p[i].productName.slice(-2);
						if (p[i].productName) {
							if (code == "服务") {
								let t = p[i].totalAmount;
								that.serveTotalAmount += parseInt(t);
								that.serveInfo.push(p[i]);
								that.showServe = true;
							}else{
								let m = p[i].depositeAmt;
								that.facilityTotalAmount += parseInt(m);
								that.facilityInfo.push(p[i]);
								that.showFacility = true;
							}
						}
					}
					that.allTotalAmount = that.serveTotalAmount + that.facilityTotalAmount;
				}
				that.caterSystem = detail.caterSystem;
				that.caterSystemName = detail.caterSystemName;
				that.install = detail.installServer;
				that.pay = detail.payServer;
				that.provinceName = detail.provinceName === ' ' ? '' : detail.provinceName;
				that.cityName = detail.cityName === ' ' ? '' : detail.cityName;
				that.areaName = detail.areaName === ' ' ? '' : detail.areaName;
				that.address = detail.address === ' ' ? '' : detail.address;
				that.longitude = detail.longitude === ' ' ? '' : detail.longitude;
				that.latitude = detail.latitude === ' ' ? '' : detail.latitude;
				that.textarea1 =  detail.environmentDesc === ' ' ? '' : detail.environmentDesc;
				that.installLinker = detail.installLinker === ' ' ? '' : detail.installLinker;
				that.linkerTel = detail.linkerTel === ' ' ? '' : detail.linkerTel;
				that.caterMerchantId = detail.caterMerchantId === ' ' ? '' : detail.caterMerchantId;
				that.textarea2 =  detail.description;
				that.value1 = detail.installDate;
				that.file = detail.files;
				
				if (detail.payServer == '1') {
					that.radio1 = 1;
					that.pay = '1';
				}else{
					that.radio1 = 2;
					that.pay = '0';
				};
				if (detail.installServer == '1') {
					that.radio2 = 3;
					that.install = 1;
				}else{
					that.radio2 = 4;
					that.install = 0;
				};
				if (that.radio2 == 3) {
					that.handleInstall = true;
					that.install = 1;
				}else{
					that.handleInstall = false;
					that.install = 0;
					that.value1 = '';
					that.installLinker = '';
					that.linkerTel = '';
					that.textarea1 = '';
				};
			}else{
				if (data.body.msg == null) {
					that.$message('数据格式错误！');
					window.history.go(-1);
				}else{
					that.$message(data.body.msg);
				}
			};
		},
		function (error) {//请求失败
			that.$message('请求超时！');
		});
    },
    methods: {
    	beforeUpload(file){//文件上传之前获取文件名
    		this.upLoadData.fileName = file.name;
    	},
	    UploadSuccess(response, file){//文件上传成功后
	    	this.dialogImageUrl = file.url;
			this.dialogVisible = true;
			this.files.push(response.data);
	    },
	    UploadError(err, file, fileList){//文件上传失败
			this.$message('文件上传失败，请检查文件大小是否超过500kb！');
	    },
		handleRemove(file, fileList) {//删除上传的文件
			let index = this.files.indexOf(file) 
			this.files.splice(index, 1);
		},
		handleAmap(){//调用高德地图
			this.showAmap = true;
			this.showAmend = false;
	    },
	    goSetup(){
	    	this.showAmap = arguments[0];
	    	let mapCenter = arguments[1];
	    	this.longitude = mapCenter[0];
	    	this.latitude = mapCenter[1];
	    	if (arguments[2]) {
	    		this.address = arguments[2];
	    	};
	    	if (arguments[3]) {
	    		this.provinceName = arguments[3];
	    	};
	    	if (arguments[4]) {
	    		this.cityName = arguments[4];
	    	};
	    	if (arguments[5]) {
	    		this.areaName = arguments[5];
	    	};
	    },
	    handleSystem() {//商家系统下拉列表
	    	let that = this;
			let option = {
				timeout: 30 * 1000 //30秒过期
			};
			let url = 'erp-api/erp/assistant/combo/list?token='+ that.token +'&grpNum=ZDY_CATER_TYPE';
			that.$http.get(url).then(function (data) {//获取部门id和name
				let info = data.body.data;
				if (info) {
					that.systems = info;
	    			that.showSystem = true;
				}else{
					that.$message(data.body.msg);
				};
			},
			function (error) {
				that.$message('请求超时！');
			});
        },
        handleEvent(event,index){//选择商家系统
        	this.caterSystemName = event.currentTarget.innerText;
        	this.showSystem = false;
        	this.caterSystem = this.systems[index].fId;
        },
		handlePay(value){//是否选择支付服务
			if (value == 1) {
        		this.pay = '1';
        	}else{
        		this.pay = '0';
        	};
		},
		handleRadio(value){//是否选择安装服务
			if (value == 3) {
				this.handleInstall = true;
				this.install = 1;
			}else{
				this.handleInstall = false;
				this.install = 0;
				this.value1 = '';
				this.installLinker = '';
				this.linkerTel = '';
				this.textarea1 = '';
			};
		},
	    handleCommand(command) {//选择商家系统
	    	this.caterSystemName = command;
        },
		handleProducts(){//选择商品
			window.localStorage.setItem('productEntity', JSON.stringify(this.productEntity));
			this.findProduct();
			this.showProduct = true; 
			this.showAmend = false;
		},
		goBack(){//选好商品后子组件返回的参数，这里接收子组件传到父组件的值
			this.showProduct = arguments[1];
			this.showAmend = true;
			if (arguments[0] == 'save') {
				this.serveInfo = [];
				this.facilityInfo = [];
				let productEntity = window.localStorage.getItem('productEntity');
				if (productEntity) {
					let p = JSON.parse(productEntity);
					this.productEntity = p;
					this.serveTotalAmount = 0;
					this.facilityTotalAmount = 0;
					for (const i in p) {
						let code = p[i].productName.slice(-2);
						if (p[i].productName) {
							if (code == "服务") {
								console.log(p[i])
								let t = p[i].totalAmount; 
								this.serveTotalAmount += parseInt(t);
								this.serveInfo.push(p[i]);
								this.showServe = true;
							}else{
								let m = p[i].depositeAmt;
								this.facilityTotalAmount += parseInt(m);
								this.facilityInfo.push(p[i]);
								this.showFacility = true;
							}
						}
					}
					this.allTotalAmount = this.serveTotalAmount + this.facilityTotalAmount;
				}
			}
		},
		findProduct(){//查询服务和设备信息
			let self = this;
			let url = '/erp-api/erp/product/list?token=5a5c6c1a-435b-44b4-93a1-1de40c6af8d3&types=2,6&cusType=003';

			self.$http.get(url, self.option).then(
				function(data) {
					// 这里是处理正确的回调
					if (data.body.msg) {
						self.$message({
							message: data.body.msg,
							type: "warning"
						});
					} else {
						let info = data.body.data;
						for (const i in info) {
							let code = info[i].number.slice(0,2);
							if (code == "FW") { 
								let ser = self.productEntity;
								for (const s in ser) {
									if (ser[s].productName == info[i].name) {
										ser[s].chargeway = info[i].chargeway;
									}
								}  
							}
						}
					}
				},
				function(error) {
					// 这里是处理错误的回调
					self.$message({
						message: "请求失败，请刷新重试！",
						type: "error"
					});
				}
			);
		},
        serTel(){//正则验证电话号码
			let value = this.$refs.serTel.value;
            let reg = /^(0|86|17951)?(13[0-9]|15[012356789]|17[678]|18[0-9]|14[57])[0-9]{8}$/;
            if (!reg.test(value)) {
                this.serTelActive = true;
            }else{
                this.serTelActive = false;
            };
		},
		telephone(){//正则验证电话号码
			let value = this.$refs.telephone.value;
            let reg = /^(0|86|17951)?(13[0-9]|15[012356789]|17[678]|18[0-9]|14[57])[0-9]{8}$/;
            if (!reg.test(value)) {
                this.telephoneActive = true;
            }else{
                this.telephoneActive = false;
            };
		},
		bankAcct(){//验证卡号为数字
			let value = this.$refs.bankAcct.value;
            let reg = /^\d+$/;
            if (!reg.test(value)) {
                this.bankActive = true;
            }else{
                this.bankActive = false;
            };
		},
		linkerTelephone(){//正则验证电话号码
			let value = this.$refs.linkerTel.value;
            let reg = /^(0|86|17951)?(13[0-9]|15[012356789]|17[678]|18[0-9]|14[57])[0-9]{8}$/;
            if (!reg.test(value)) {
                this.linkerTelActive = true;
            }else{
                this.linkerTelActive = false;
            };
		},
        handleSave(){//保存商品订单
        	let that = this;
	    	let d = new Date(that.value1);
			let date = '';
        	if (that.value1) {//将世界标准时间转为aa-bb-cc格式 
        		date = d.getFullYear() + '-' + (d.getMonth() + 1) + '-' + d.getDate();
    		}else{
    			date = ' ';
    		};
			let option = {
				timeout: 30 * 1000 //30秒过期
			};
			let url = '/erp-api/erp/sale/order/submit/estate/info';
			let params = {
				"status": 0,
				"userId": that.userId,  
				"token": that.token,
				"jsonString": JSON.stringify({
					"matchData": 1,
					"cusTypeNumber": "5699aa52ec7e62",
					"caterSystem": that.caterSystem,
					"caterSystemName": that.caterSystemName,
					"caterMerchantId": that.$refs.caterMerchantId.value,
					"cusName": that.$refs.cusName.value,
					"address": that.$refs.address.value,
					"productEntity": that.productEntity,
					"acctName": that.$refs.acctName.value,
					"dept": {
						"id": that.id,
						"name": that.name
					},
					"telephone": that.$refs.telephone.value,
					"latitude": that.$refs.latitude.value,
					"longitude": that.$refs.longitude.value,
					"position": that.$refs.position.value,
					"fId": that.fId,
					"linker": that.$refs.linker.value,
					"files": that.files,
					"isExistProject": "0",
					"openBank": that.$refs.openBank.value,
					"prjId": "",
					"proName": that.$refs.proName.value,
					"provinceName": that.$refs.provinceName.value,
					"cityName": that.$refs.cityName.value,
					"areaName": that.$refs.areaName.value,
					"serTel": that.$refs.serTel.value,
					"bankAcct": that.$refs.bankAcct.value,
					"cooperate": "",
					"saleModel": "BZ",
    				"description": that.textarea2,
					"payServer": that.pay,
					"installServer": that.install,
					"installDate": date,
    				"installLinker": that.installLinker,
					"linkerTel": that.linkerTel,
					"environmentDesc": that.textarea1,
				})
			};
			if (!that.$refs.proName.value) {//验证必填项
                that.proActive = true;
                that.$message('项目名称不能为空！');
            }else if(!that.$refs.cusName.value){
                that.cusActive = true;
                that.$message('公司名称不能为空！');
            }else if(!that.$refs.provinceName.value){
                that.provinceActive = true;
                that.$message('所属省份不能为空！');
            }else if(!that.$refs.cityName.value){
                that.cityActive = true;
                that.$message('所属城市不能为空！');
            }else if(!that.$refs.areaName.value){
                that.areaActive = true;
                that.$message('所属区域不能为空！');
            }else if(!that.$refs.address.value){
                that.addressActive = true;
                that.$message('详细地址不能为空！');
            }else if(!that.$refs.serTel.value){
                that.serTelActive = true;
                that.$message('固定电话不能为空！');
            }else if(!that.$refs.linker.value){
                that.linkerActive = true;
                that.$message('联系人不能为空！');
            }else if(!that.$refs.position.value){
                that.positionActive = true;
                that.$message('联系人职务不能为空！');
            }else if(!that.$refs.telephone.value){
                that.telephoneActive = true;
                that.$message('移动电话不能为空！');
            }else if(!that.caterSystemName){
                that.caterActive = true;
                that.$message('请选择商家系统！');
            }else if(!that.productEntity){
                that.$message('请选择商品！');
            }else{
				if(that.radio2 == 3){
                	if(!that.value1){
	                	that.$message('请选择安装服务日期！');
	                	return;
	            	}else if(!that.installLinker){
	               	 	that.installLinkerActive = true;
	                	that.$message('安装对接人不能为空！');
	                	return;
	            	}else if(!that.linkerTel){
	               	 	that.linkerTelActive = true;
	                	that.$message('对接人电话不能为空！');
	                	return;
	            	}
            	}
				that.disabled = true;
            	that.loading = true;
            	that.$http.post(url, params, option).then(function (data) {
            		let info = data.body.data;
            		if (info) {
            			that.loading = false;
						that.disabled = false;
						that.$router.push({path:'/'});
            		}else{
            			that.loading = false;
						that.disabled = false;
						that.$message(data.body.msg);
					};
				},
				function (error) {
					that.$message('保存失败！');
					that.loading = false;
					that.disabled = false;
				})
            };
        },
        handlePrimary(){//提交商品订单
			let that = this;
	    	let d = new Date(that.value1);
			let date = '';
        	if (that.value1) {//将世界标准时间转为aa-bb-cc格式 
        		date = d.getFullYear() + '-' + (d.getMonth() + 1) + '-' + d.getDate();
    		}else{
    			date = ' ';
    		};
			let option = {
				timeout: 30 * 1000 //30秒过期
			};
			let url = '/erp-api/erp/sale/order/submit/estate/info';
			let params = {
				"status": 1, 
				"userId": that.userId,  
				"token": that.token,
				"jsonString": JSON.stringify({
					"matchData": 1,
					"cusTypeNumber": "5699aa52ec7e62",
					"caterSystem": that.caterSystem,
					"caterSystemName": that.caterSystemName,
					"caterMerchantId": that.$refs.caterMerchantId.value,
					"cusName": that.$refs.cusName.value,
					"address": that.$refs.address.value,
					"productEntity": that.productEntity,
					"acctName": that.$refs.acctName.value,
					"dept": {
						"id": that.id,
						"name": that.name
					},
					"telephone": that.$refs.telephone.value,
					"latitude": that.$refs.latitude.value,
					"longitude": that.$refs.longitude.value,
					"position": that.$refs.position.value,
					"fId": that.fId,
					"linker": that.$refs.linker.value,
					"files": that.files,
					"isExistProject": "0",
					"openBank": that.$refs.openBank.value,
					"prjId": "",
					"proName": that.$refs.proName.value,
					"provinceName": that.$refs.provinceName.value,
					"cityName": that.$refs.cityName.value,
					"areaName": that.$refs.areaName.value,
					"serTel": that.$refs.serTel.value,
					"bankAcct": that.$refs.bankAcct.value,
					"cooperate": "",
					"saleModel": "BZ",
    				"description": that.textarea2,
					"payServer": that.pay,
					"installServer": that.install,
					"installDate": date,
    				"installLinker": that.installLinker,
					"linkerTel": that.linkerTel,
					"environmentDesc": that.textarea1,
				})
			};
			if (!that.$refs.proName.value) {//验证必填项
                that.proActive = true;
                that.$message('项目名称不能为空！');
            }else if(!that.$refs.cusName.value){
                that.cusActive = true;
                that.$message('公司名称不能为空！');
            }else if(!that.$refs.provinceName.value){
                that.provinceActive = true;
                that.$message('所属省份不能为空！');
            }else if(!that.$refs.cityName.value){
                that.cityActive = true;
                that.$message('所属城市不能为空！');
            }else if(!that.$refs.areaName.value){
                that.areaActive = true;
                that.$message('所属区域不能为空！');
            }else if(!that.$refs.address.value){
                that.addressActive = true;
                that.$message('详细地址不能为空！');
            }else if(!that.$refs.serTel.value){
                that.serTelActive = true;
                that.$message('固定电话不能为空！');
            }else if(!that.$refs.linker.value){
                that.linkerActive = true;
                that.$message('联系人不能为空！');
            }else if(!that.$refs.position.value){
                that.positionActive = true;
                that.$message('联系人职务不能为空！');
            }else if(!that.$refs.telephone.value){
                that.telephoneActive = true;
                that.$message('移动电话不能为空！');
            }else if(!that.caterSystemName){
                that.caterActive = true;
                that.$message('请选择商家系统！');
            }else if(!that.productEntity){
                that.$message('请选择商品！');
            }else{
            	if(that.radio2 == 3){
                	if(!that.value1){
	                	that.$message('请选择安装服务日期！');
	                	return;
	            	}else if(!that.installLinker){
	               	 	that.installLinkerActive = true;
	                	that.$message('安装对接人不能为空！');
	                	return;
	            	}else if(!that.linkerTel){
	               	 	that.linkerTelActive = true;
	                	that.$message('对接人电话不能为空！');
	                	return;
	            	}
            	}
				that.disabled = true;
				that.primaryLoading = true;
            	that.$http.post(url, params, option).then(function (data) {
            		let info = data.body.data;
            		if (info) {
            			that.primaryLoading = false;
						that.disabled = false;
						that.$router.push({path:'/'});
            		}else{
            			that.primaryLoading = false;
						that.disabled = false;
						that.$message(data.body.msg);
					};
				},
				function (error) {
					that.$message('提交失败');
					that.disabled = false;
					that.primaryLoading = false;
				})
            };
        }
    }
}
</script>

<style lang="less">
	.amend{
		width: 100%;
		background: #E6EFFB;

		.error{
			background: #FFF1F1 !important;
			border: 1px solid #FF6600;
		}
		.top{
		    width: 100%;
		    height: 50px;
		    color: #fff;
		    background: #20A0FF;
		    line-height: 50px;
		    text-align: center;
		    position: fixed;
		    top: 0;
		    left: 0;
		    z-index: 9;

		    .goBack{
		      	float: left;
		      	height: 100%;
		      	line-height: 50px;
			    position: absolute;
			    top: 0;
			    left: 20px;
		    }

		    #set_up{
		    	width: 20%;
		    	height: 100%;
			    float: right;
			    position: absolute;
			    top: 0;
			    left: 80%;
		    }
		}

		.active{
			display: block;
		}
		.title{
			overflow: hidden;
			padding: 3px 0;
			line-height: 30px;

			span{
				color: #0093E2;
				font-size: 12px;
				float: left;
			}
			i{
				width: 50px;
				height: 24px;
				color: #fff;
				background: #20A0FF;
				font-size: 12px;
				text-align: center;
				line-height: 24px;
				border-radius: 5px;
				float: right;
				margin: 2px 10px 0 0;
			}
		}
		
		.first,.product,.card{
			color: #aaa;
			font-size: 14px;
			overflow: hidden;
			padding: 10px;
			background: #fff;
			margin-bottom: 8px;
			
			h3{
				color: #0093E2;
				font-size: 12px;
				font-weight: normal;
				text-align: left;
				border-bottom:  2px solid #E6EFFB;
				padding-bottom: 10px;
			}
		}
		.first{
			p{
				width: 100%;
				line-height: 40px;

				i{
					text-align: left;
					margin-left: 10px;
					display: block;
				}
				span{
					overflow: hidden;
					white-space: nowrap;
					text-overflow: ellipsis;
					text-align: left;
					border-bottom: 1px solid #E6EFFB;
					display: block;
					padding: 0 10px;
				}
			}
		}

		.product{
			.productBox{
				ul{
					b{
						text-align: left;
						line-height: 40px;
						color: #777;
						border-bottom: 1px solid #E6EFFB;
						display: block;
						padding-left: 6px;
					}
					li{
						p{
							height: 40px;
							line-height: 40px;
							padding: 0 10px;
							display: block;
							i{
								float: left;
							}
							span{
								float: right;
							}
						}
					}
					strong{
						text-align: right;
						display: block;
					}
				}
			}
			.foot{
				height: 40px;
				line-height: 40px;
				padding: 0 10px;

				strong{
					text-align: right;
					display: block;
				}
				b{
					width: 50px;
					height: 26px;
					color: #fff;
					background: #20A0FF;
					line-height: 26px;
					border-radius: 5px;
					float: right;
					margin-top: 8px;
				}
			}
		}
		.card{
			.checkeBox{
				display: flex;
				justify-content:space-between;
				margin: 16px 4px;
			}
			.drop{
				width: 88%;
				margin: 0 auto;

				.dropBox{
					p{
						width: 88%;
						height: 40px;
						line-height: 40px;
						border-bottom: 1px solid #bfcbd9;
						margin: 0 auto;
					}
				}
			}
			input{
				width: 88%;
				height: 40px;
				border: 1px solid #bfcbd9;
				outline:none;
				border-radius: 4px;
				padding-left: 10px;
				margin-top: 10px;
			}
			.el-dropdown-menu{
				width: 100%;
			}
			.dropBox{
				position: relative;
				
				span{
					display: block;
					height: 40px;
					line-height: 40px;
					border-bottom: 1px solid #E6EFFB;
					padding: 0 15px;
				}
				.border{
					border-bottom: 1px solid #EA4335;
				}
				ul{
					width: 60%;
					overflow: hidden;
					background: #fff;
					border: 1px solid #d1dbe5;
					box-shadow: 0 2px 4px rgba(0,0,0,.12), 0 0 6px rgba(0,0,0,.12);
					margin: 10px auto 0;

					li{
						height: 40px;
						line-height: 36px;
					}
					li:hover{
						background: #E6E6E6;
					}
				}
			}
		}

		.box{
			color: #aaa;
			font-size: 14px;
			overflow: hidden;
			padding: 10px;
			background: #fff;
			margin-bottom: 8px;
			
			p{
				color: #0093E2;
				font-size: 12px;
				text-align: left;
			}
			input{
				width: 88%;
				height: 40px;
				border: 1px solid #bfcbd9;
				outline:none;
				border-radius: 4px;
				padding-left: 10px;
				margin-top: 10px;
				box-sizing: border-box;
			}
			textarea{
				width: 88%;
				height: 60px;
				resize: none;
				outline:none;
				border-radius: 4px;
				margin-top: 10px;
				padding: 6px 10px;
				box-sizing: border-box;
			}
			.span{
				float: left;
				margin: 10px 0 0 14px;
			}
			.el-radio{
				margin: 10px 0 0 54px;
				cursor: auto;
			}
			.el-radio__input{
				cursor: auto;
			}
			.el-upload{
				width: 40%;
				height: 150px;
				float: left;
				margin: 10px 0 0 6%;
			}
			.el-upload-list{
				li{
					width: 40%;
					height: 150px;
					float: left;
					margin: 10px 0 0 6%;
				}
			}
			.el-dialog{
				width: 70%;
			}
			.imgBox{
				display: inline;
				li{
					width: 40%;
					height: 150px;
					float: left;
					margin: 10px 0 0 6%;
					
					img{
						width: 100%;
						height: 100%;
						border-radius: 6px;
					}
				}
			}
		}
		.submit{
			width: 100%;
			height: 45px;
			font-size: 16px;
			border-radius: 0;
			position: fixed;
			bottom: 0;
			left: 0;
			z-index: 9;
		}
		.install{
			.block{
				.el-date-editor{
					width: 100%;
					height: 40px;
					position: relative;
					margin-top: 10px;

					.el-input__icon{
						right: 30px;
					}
					.el-input__inner{
						margin: 0;
					}
				}
			}
		}
	}
</style>
